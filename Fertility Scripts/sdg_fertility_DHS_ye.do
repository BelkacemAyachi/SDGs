...
clear
set more off
cd "C:\Users\10064142\Desktop\SDGs Publication\Yemen\DHS 2013"
...
if (`m'==11) keep if s105 == 0		/*** No education ***/
if (`m'==12) keep if s105 == 1 | s105 == 2 | s105 == 3		/*** Primary ***/
if (`m'==13) keep if s105 == 4								/*** Secondary ***/
if (`m'==14) keep if s105 == 5 | s105 == 6 					/*** Higher ***/
...
forvalues i = 0/4 {
	...
	tabout age5 using "ye_fertility\births`i'.txt" if colper == `i', svy style(csv) format(7) h1(nil) h2(nil) h3(nil) replace
}

*
* Compile and save to "births.dta"
forvalues i = 0/4 {
	insheet using "ye_fertility\births`i'.txt", delimiter(",") clear
	gen colper = `i'
	rename v1 age5
	rename v2 births
	drop if age5 == "Total"
	save "ye_fertility\births`i'.dta", replace
	erase "ye_fertility\births`i'.txt"
}
use "ye_fertility\births0.dta", clear
forvalues i = 1/4 {
append using "ye_fertility\births`i'.dta"
erase "ye_fertility\births`i'.dta"
}
erase "ye_fertility\births0.dta"
sort colper age5
label define colper 0 "0-2" 1 "3-5" 2 "6-8" 3 "9-11" 4 "12-14"
label value colper colper
order colper age5 births
save "ye_fertility\births.dta", replace
...
forvalues i = 0/4 {
	...
	* Output high end exposure to aggregate file; women younger than 15 are excluded.
	...
	tabout age5 highexp if age5 > 0 using "ye_fertility\highexp`i'.txt", svy style(csv) format(7) h1(nil) h2(nil) h3(nil) replace
	...
	* Output low end exposure to aggregate file; women younger than 20 are excluded.
	...
	tabout age5 lowexp if age5 > 0 & lowexp > 0 using "ye_fertility\lowexp`i'.txt", svy style(csv) format(7) h1(nil) h2(nil) h3(nil) replace
...
forvalues i = 0/4 {
	insheet using "ye_fertility\highexp`i'.txt", delimiter(",") clear
	quietly describe
	gen vlast = v`r(k)'
	keep v1 vlast
	drop if v1 == "Total" | v1 == ""
	...
	save "ye_fertility\highexp`i'.dta", replace
	erase "ye_fertility\highexp`i'.txt"
	insheet using "ye_fertility\lowexp`i'.txt", delimiter(",") clear
	quietly describe
	gen vlast = v`r(k)'
	keep v1 vlast
	drop if v1 == "Total" | v1 == ""
	...
	rename (v1 vlast) (age5 exposure)
	order colper age5 exposure
	save "ye_fertility\lowexp`i'.dta", replace
	erase "ye_fertility\lowexp`i'.txt"
}

use "ye_fertility\highexp0.dta", clear
append using "ye_fertility\lowexp0.dta"
forvalues i = 1/4 {
append using "ye_fertility\highexp`i'.dta"
append using "ye_fertility\lowexp`i'.dta"
erase "ye_fertility\highexp`i'.dta"
erase "ye_fertility\lowexp`i'.dta"
}
erase "ye_fertility\highexp0.dta"
erase "ye_fertility\lowexp0.dta"
collapse (sum) exposure, by(colper age5)
replace exposure = exposure / 12
save "ye_fertility\exposure.dta", replace
...
use "ye_fertility\births.dta", clear
merge 1:1 colper age5 using "ye_fertility\exposure.dta"
drop if _merge != 3
drop _merge
sort colper age5
drop if age5 == "10-14"
...
*save "ye_fertility\fertility_gov`zinou'", replace
save "ye_fertility\fertility_`m'", replace
