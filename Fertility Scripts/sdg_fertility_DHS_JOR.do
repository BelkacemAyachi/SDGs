...
clear
set more off
cd "C:\Users\zinou\Desktop\SDGs Publication\Jordan\DHS 2012"
...
use "Datasets\JOBR6CFL.dta", clear
gen maxper = 5
gen period = 24
...
forvalues i = 0/4 {
	...
	tabout age5 using "jo_fertility\births`i'.txt" if colper == `i', svy style(csv) format(7) h1(nil) h2(nil) h3(nil) replace
}

*
* Compile and save to "births.dta"
forvalues i = 0/4 {
	insheet using "jo_fertility\births`i'.txt", delimiter(",") clear
	gen colper = `i'
	rename v1 age5
	rename v2 births
	drop if age5 == "Total"
	save "jo_fertility\births`i'.dta", replace
	erase "jo_fertility\births`i'.txt"
}
use "jo_fertility\births0.dta", clear
forvalues i = 1/4 {
append using "jo_fertility\births`i'.dta"
erase "jo_fertility\births`i'.dta"
}
erase "jo_fertility\births0.dta"
sort colper age5
label define colper 0 "0-2" 1 "3-5" 2 "6-8" 3 "9-11" 4 "12-14"
label value colper colper
order colper age5 births
save "jo_fertility\births.dta", replace
...
set more off
use "Datasets\JOPR6CFL.dta", clear
gen maxper = 5
gen period = 36
...
forvalues i = 0/4 {
	* Set upper limits for date of analysis period
	replace upplim = hv008 - (period * colper) - 1
	* Calculate age of woman and her 5-year age group
	replace age = upplim - hv011
	...
	* Output high end exposure to aggregate file; women younger than 15 are excluded.
	...
	tabout age5 highexp if age5 > 0 using "jo_fertility\highexp`i'.txt", svy style(csv) format(7) h1(nil) h2(nil) h3(nil) replace
	...
	* Output low end exposure to aggregate file; women younger than 20 are excluded.
	...
	tabout age5 lowexp if age5 > 0 & lowexp > 0 using "jo_fertility\lowexp`i'.txt", svy style(csv) format(7) h1(nil) h2(nil) h3(nil) replace
...
forvalues i = 0/4 {
	insheet using "jo_fertility\highexp`i'.txt", delimiter(",") clear
	quietly describe
	gen vlast = v`r(k)'
	keep v1 vlast
	drop if v1 == "Total" | v1 == ""
	...
	save "jo_fertility\highexp`i'.dta", replace
	erase "jo_fertility\highexp`i'.txt"
	insheet using "jo_fertility\lowexp`i'.txt", delimiter(",") clear
	quietly describe
	gen vlast = v`r(k)'
	keep v1 vlast
	drop if v1 == "Total" | v1 == ""
	...
	rename (v1 vlast) (age5 exposure)
	order colper age5 exposure
	save "jo_fertility\lowexp`i'.dta", replace
	erase "jo_fertility\lowexp`i'.txt"
}

use "jo_fertility\highexp0.dta", clear
append using "jo_fertility\lowexp0.dta"
forvalues i = 1/4 {
append using "jo_fertility\highexp`i'.dta"
append using "jo_fertility\lowexp`i'.dta"
erase "jo_fertility\highexp`i'.dta"
erase "jo_fertility\lowexp`i'.dta"
}
erase "jo_fertility\highexp0.dta"
erase "jo_fertility\lowexp0.dta"
collapse (sum) exposure, by(colper age5)
replace exposure = exposure / 12
save "jo_fertility\exposure.dta", replace
...
use "jo_fertility\births.dta", clear
merge 1:1 colper age5 using "jo_fertility\exposure.dta"
drop if _merge != 3
drop _merge
sort colper age5
drop if age5 == "10-14"
...
save "jo_fertility\fertility_`m'", replace
